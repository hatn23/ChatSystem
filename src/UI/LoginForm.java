...
public class LoginForm extends javax.swing.JFrame {
	...
	protected void jButtonLoginActionPerformed(java.awt.event.ActionEvent evt) throws SQLException {//GEN-FIRST:event_jButtonLoginActionPerformed
		if (this.jTextFieldPseudo.getText().equals("")) {
			...
			try {
				...
				/* if (this.inter.checkPseudo()) {
                    try {
                    	if(Database.Ip_adress_existant(this.inter.getUser().getHost())) {
                    		if
                        }
                    	else {
				...
				//Si @IP existe ds la database
				if (Database.Ip_adress_existant(this.inter.getUser().getHost())) {
					String pseudo =Database.get_Username(inter.getUser().getHost());
					if(this.inter.getUser().getPseudo().equals(pseudo) ) {
						new UDPClientThread().sendBroadcast(this.inter);
						this.inter.getHome().display();
						this.setVisible(false);
						setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
						...
						new UDPClientThread().sendDisconnect(this.inter);
						this.jLabelWarning.setText("WARNING : Wrong Username!");
					}
				}
				else {
					if (this.inter.checkPseudo()) {
...
private void jTextFieldPseudoKeyPressed(java.awt.event.KeyEvent evt) throws SQLException {//GEN-FIRST:event_jTextFieldPseudoKeyPressed
